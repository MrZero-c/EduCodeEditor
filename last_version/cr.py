...
def toplevel(event = None):
    ...
    try:
        ...
        label.config(text = ','.join(args))
        label.pack()
    except Exception as e:
        print(f"error = {e}")

def get_visible_text(vtxt):
    start_index = vtxt.index("@0,0")
    end_index = vtxt.index(f"@0,{vtxt.winfo_height()}")
    return start_index, end_index

def highlight(event=None):
    global color
    start_vis, end_vis = get_visible_text(txt)

    for i in color.keys():
        txt.tag_remove(i,start_vis,end_vis)
 
    for word,col in color.items():
            txt.tag_configure(word,foreground=col)
            start = start_vis
            while True:
                start = txt.search(fr'\y{word}\y',start,stopindex = end_vis, regexp = True)
                if not start:
                    break
                stop = f'{start}+{len(word)}c'
                txt.tag_add(word,start,stop)
                start = stop
...
def start_highlight(): 
    ...
    for word,col in color.items():
            ...
            start = "1.0"
            while True:
                start = txt.search(fr'\y{word}\y',start,stopindex = tk.END, regexp = True)
                if not start:
                    break
                stop = f'{start}+{len(word)}c'
                txt.tag_add(word,start,stop)
...
def terminal():
    ...
    try:
        res_txt.delete(f"{prompt_index + 2}.0", tk.END)
...
root.bind('<F1>', openfile)  # F1 - Открыть файл
root.bind('<F2>', newfile)   #F2 - Создать новый файл
root.bind('<F3>', savefile)  # F3 - Сохранить файл
root.bind('<F5>', run)  # F5 - Запустить скрипт
root.bind('<parenleft>',toplevel)
root.bind('<Return>', hide_hint)     
root.bind('<parenright>', hide_hint) 
root.bind('<Button-1>', hide_hint)
root.bind('<KeyRelease>',highlight)
root.bind('<BackSpace>', hide_hint)
