...
def toplevel(event = None):
    ...
    try:
        name_funk = re.split(r'[., ,=,+]',string)[-1].split('(')[0]
        help_txt = pydoc.render_doc(name_funk, "Help on %s")
        help_txt = re.sub(r'.\x08','',help_txt)
        obj = eval(name_funk)
        if obj.__doc__:
            help_txt =obj.__doc__
        else:
            None
